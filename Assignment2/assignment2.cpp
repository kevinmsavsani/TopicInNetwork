...
void calculate_mean_sd()            // function to calculate mean and SD of Packet Delays
{
	double sum=0;
	double mean;
	double sd=0;
...
void INQ()
{
	...
	calculate_mean_sd();
	link_u=link_u/(T*N);    // Calculating Link Utilisation 
...
void KOUQ(){
	...
	for (int i = 0; i < T; ++i)
	{
		...
		{
			if(numOut[k]>K)
				KOUQ_drop_prob++;        // increment when a port have one than K packets
		}
	...
	KOUQ_drop_prob=(KOUQ_drop_prob/N)/T;   // Calculating KOUQ Drop Probability
...
void iSLIP(){
	...
	calculate_mean_sd();         // Call to calculate Avg. Delay and SD
	link_u=link_u/(T*N);        //calculating link Utilisation
